  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const pendingProduct = localStorage.getItem('pendingAddProduct')
        if (pendingProduct) {
          try {
            let quantity = 1
            const res = await API.post('/cart/add-items-to-cart', { productId:pendingProduct, quantity })
            a.handlecart()
            localStorage.removeItem('pendingAddProduct')
          } catch (error) {
            console.error(error)
          }
        }
  ...
  return (
    <div className='login'>
      <form className='loginForm' onSubmit={handleSubmit}>
        ...
        <button type="submit" className="btn  btn-sm mt-4">LogIn</button>
